  ...
  <div class="container mt-5 d-flex justify-content-center">
    ...
    <div class="card p-4 m-3 col-6" v-for="(item,idx) in member" :key="idx">
        <div class="first" align="right" style="margin-right:10px">
            <span>{{item.userAge}}</span> <span>{{item.userGender}}</span><button v-if="item.userNo === myNum" @click="delCard(item.mateListNo)">x</button> 
    ...
    <b-button v-if="mate.userNo === myNum" @click="mateDefine">모집마감</b-button>
...
export default {
  ...
  setup(props,) {
    ...
    const join = () => {
      ...
      if (props.mateDetail.memberlimit < Number(friendnum.value) + Number(memberSum.value)) {
        console.log(props.mateDetail.memberlimit)
    console.log(Number(friendnum.value) + Number(memberSum.value))
        alert('인원초과 수고')
      } else if (Number(friendnum.value) > props.mateDetail.friendlimit) { alert('인원초과 수고')} else {
        if (check.value === 0) {
